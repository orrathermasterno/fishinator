...
uint64_t Attacks::generate_magics(SliderPiece piece, Square sq) {
    // 2^12 -- max possible amount of blocker combinations (for rooks in the corners)
    Bitboard blockers[4096]; // occupancy mask power set 
    Bitboard reference[4096]; // holds attack bbs corresponding to blockers at same index
    uint64_t used [4096] ;
    Bitboard mask = generate_sliding_mask(piece, sq);
    ...
    for (int attempt = 0; attempt < 100000000; attempt++) {
        ...
        for (int i = 0 ; i < (1 << slider_bits[piece][sq]); i++) used[i] = 0 ;
        fail = false;
        for (int i = 0; !fail && i < (1 << slider_bits[piece][sq]); i ++) {
            index = (int)((blockers[i] * magic_number) >> (64 - slider_bits[piece][sq]));
            if (used [index] == 0) used[index ] = reference[i];
            else if (used[index] != reference[i]) fail = true;
        }
...
template uint64_t Attacks::generate_magics<Xorshift>(SliderPiece, Square);